...
});this.roleList.change(function(evt){that.roleList_changeHandler();});this.userRoleList.jqGrid("setGridParam",{onSelectRow:function(id){that.userRoleList_changeHandler(id);
...
});this.department.focus(function(evt){that.field_focusHandler(evt);});this.submitButton.click(function(evt){that.submit_clickHandler(evt);
});this.cancelButton.click(function(evt){that.cancel_clickHandler(evt);});},fillList:function(deptEnumList){var htmlList="";
for(var i=0;i<deptEnumList.length;i++){var deptEnum=deptEnumList[i];var valueAttr='value="'+deptEnum.ordinal+'"';
var selectedAttr="";if(this.user&&deptEnum.equals(this.user.department)){selectedAttr="selected";
}if(!this.user&&deptEnum.equals(DeptEnum.NONE_SELECTED)){selectedAttr="selected";
...
break;}},submit_clickHandler:function(){this.updateUser();if(this.getErrors()){return;
...
},field_focusHandler:function(evt){this.setFieldError($(evt.target).attr("class"),false);
...
var deptEnumList=DeptEnum.getComboList();var department=deptEnumList[selected];if(DeptEnum.NONE_SELECTED.equals(department)){this.setFieldError("department",error=true);
...
this.deleteButton.button("disable");},configureListeners:function(){var that=this;
...
},getRolePanel:function(){return this.viewComponent;},onAddRole:function(event){this.roleProxy.addRoleToUser(this.getRolePanel().getUser(),this.getRolePanel().getSelectedRole());
...
break;}}});var UserFormMediator=Objs("org.puremvc.js.demos.objs.employeeadmin.view.UserFormMediator",Mediator,{initialize:function(name,viewComponent){UserFormMediator.$super.initialize.call(this,name,viewComponent);
...
this.userProxy=this.facade.retrieveProxy(ProxyNames.USER_PROXY);},userProxy:null,getUserForm:function(){return this.viewComponent;
},onAdd:function(event){var user=this.getUserForm().getUser();this.userProxy.addItem(user);
this.sendNotification(NotificationNames.USER_ADDED,user);var userForm=this.getUserForm();
...
userForm.setFocus();break;}}});UserFormMediator.ADD="add";UserFormMediator.UPDATE="update";
UserFormMediator.CANCEL="cancel";UserFormMediator.MODE_ADD="modeAdd";UserFormMediator.MODE_EDIT="modeEdit";
var UserListMediator=Objs("org.puremvc.js.demos.objs.employeeadmin.view.UserListMediator",Mediator,{userList:null,initialize:function(name,viewComponent){UserListMediator.$super.initialize.call(this,name,viewComponent);
var userList=this.getUserList();userList.addEventListener(UserList.NEW,this.onNew,this);
userList.addEventListener(UserList.DELETE,this.onDelete,this);userList.addEventListener(UserList.SELECT,this.onSelect,this);
var userProxy=this.facade.retrieveProxy(ProxyNames.USER_PROXY);userList.setUsers(userProxy.getUsers());
},getUserList:function(){return this.viewComponent;},listNotificationInterests:function(){return[NotificationNames.CANCEL_SELECTED,NotificationNames.USER_UPDATED,NotificationNames.USER_ADDED,NotificationNames.USER_DELETED];
